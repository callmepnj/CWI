...
export default async function LiveNewsroomPage() {
  ...
  const sources = uniqueSources(items).slice(0, 18);
  ...
  return (
    <main className="bg-paper text-ink">
      ...
      <section className="border-b border-line bg-white">
        <div className="mx-auto grid max-w-7xl gap-8 px-4 py-12 sm:px-6 lg:grid-cols-[1.05fr_0.95fr] lg:px-8 lg:py-16">
          <div>
            ...
            <p className="mt-5 max-w-3xl text-xl font-semibold leading-8 text-ink/72">
              Latest source-backed updates, public advisories, verified context, and India Unanswered Files coverage from Cockroach Watch India.
            </p>
      ...
      <NewsroomSection title="Breaking / Developing" subtitle="Reported, developing, and public-advisory updates that need careful reading.">
      ...
      <section id="india-unanswered-files" className="border-y border-line bg-white">
        <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
          <p className="font-mono text-xs font-black uppercase tracking-[0.16em] text-royal">Public memory</p>
          <h2 className="mt-2 font-display text-4xl font-black uppercase tracking-[-0.04em] text-ink">India Unanswered Files</h2>
          <p className="mt-3 max-w-3xl leading-7 text-ink/68">Public issues, civic memory, and questions that still need answers.</p>
      ...
      <NewsroomSection title="Public Advisories" subtitle="Caution notes, digital safety, platform restriction context, and verification warnings.">
          ...
          <div className="mt-7 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            {sources.map((source) => (
              <a key={`${source.name}-${source.url}`} href={source.url} target="_blank" rel="noreferrer" className="rounded-3xl border border-line bg-white p-5 shadow-card transition hover:border-royal/35">
                <p className="font-mono text-[0.68rem] font-black uppercase tracking-[0.14em] text-royal">{source.outlet}</p>
                <h3 className="mt-2 font-display text-xl font-black uppercase leading-tight tracking-[-0.02em]">{source.name}</h3>
                <p className="mt-2 text-sm font-semibold leading-6 text-ink/62">{source.note}</p>
              </a>
            ))}
          </div>
        </div>
  ...
  return (
    <section className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
      <div className="mb-7 flex flex-wrap items-end justify-between gap-4">
        <div>
          <p className="font-mono text-xs font-black uppercase tracking-[0.16em] text-royal">CWI Live Newsroom</p>
