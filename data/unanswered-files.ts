...
export function getFileVisuals(file: UnansweredFile): FileVisual[] {
  const folder = (unansweredFileImageFolders as Record<string, string>)[file.slug];
  const images = (unansweredFileImages as Record<string, UnansweredFileImage[]>)[file.slug] ?? [];

  if (!folder || images.length < 11) {
    throw new Error(`Missing required local images for unanswered file: ${file.slug}`);
  }
