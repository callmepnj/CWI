...
async function buildAdminDashboardData() {
  ...
  const [
    ...
    `).catch(() => ({ rows: [] })),
    pool.query(`
      select id::text, article_slug as article, name, comment, status, created_at, 'Archive' as source
...
async function createUiuxAudit() {
  const audits = [
    ...
    ["Archive articles", "Check mobile spacing around source cards and rating widget.", "Medium", "Keep CTA buttons tappable."],
...
async function createStandaloneArticleDraft() {
  ...
  await createApprovalQueueItem({
    topic: post.title,
    type: "Article Draft",
    summary: "CWI Desk Writer prepared a template-based review draft for human review.",
    verificationStatus: post.verificationStatus,
    riskLevel: "Medium",
    sourceCount: post.sources.length,
    articleDraftId: article.id,
...
async function createSeoPack(articleDraftId: string | null, topic: string, slug: string) {
  const canonicalUrl = `${site.url}/live-newsroom/${slug}`;
  const result = await getPool().query<{ id: string }>(
    `
    ...
    [
      ...
      `Cockroach Watch India Live Newsroom explains ${topic}, what is known, what remains unclear, and why CWI is tracking this public-interest update.`,
      slug,
      canonicalUrl,
      ...
      JSON.stringify(["/", "/watch", "/live-newsroom", "/archive", "/india-unanswered-files", "/submit"]),
