...
- 2026-05-25 admin dashboard speed fix in progress: user reported `/admin` sections stay on "Loading CWI AI OS dashboard..." too long after clicking sidebar items. Root cause is route navigation remounting `AdminDashboard` and refetching `/api/admin/dashboard`, while the backend repeatedly checks/seeds admin DB tables. Implemented client-side section switching with history pushState, forced refresh only when needed, dashboard request timeout messaging, in-memory dashboard cache, one-time admin seed promise, and schema readiness probing before running expensive admin SQL setup.
