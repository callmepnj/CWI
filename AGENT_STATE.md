...
- Required disclaimer text: â€œCockroach Watch India is an independent civic watch, satire, and commentary platform. Not affiliated with any political party or organization unless officially declared.â€
...
- Production domain/canonical: `https://cockroachwatchindia.online`. Middleware redirects apex `cockroachwatchindia.online`, legacy Vercel host, and old Unanswered Files paths to the canonical host/path.
...
- Verification after fixes: `npm run lint`, `npm run typecheck`, `npm run validate:unanswered-files`, and `npm run build` all pass. Build generated `public/sitemap.xml` with 254 URLs and 322 app pages/routes; `/watch-desk` is now dynamic (`Æ’`) so DB-published articles can appear without a static rebuild.
...
2026-05-31 Join Now + newsroom upgrade implementation:
- Implemented global movable `FloatingJoinButton` mounted in `app/layout.tsx`, with blue/amber glow, horizontal drag/snap, localStorage side persistence, keyboard Escape close, support QR modal, and community/support CTAs.
- Added config-backed Join settings at `lib/config/join.ts`, reusable `JoinNowTrigger`, navbar Join Now CTA, homepage Join the Watch section, footer Join CTA cleanup, and read-only admin settings route `/admin/settings/join`.
- Added curated approval-first newsroom queue in `data/live-newsroom-pending.ts` for CJP X withholding, CJP account-access allegations, Delhi HC/MeitY review committee report, and CBSE fake-rumour advisory. These are not public Live Newsroom articles until human approval.
- Admin Live Newsroom now has a Pending queue tab showing curated records and source/risk/approval context.
- Switched canonical site URL and generated SEO artifacts to apex `https://cockroachwatchindia.online`; middleware now redirects `www` and old Vercel host fragments to apex while preserving legacy route redirects.
- Fixed India Unanswered Files validator path mismatch for the Ladakh image folder; validation now passes with hero/thumbnail/OG/gallery assets found.
- Removed visible watchlist/placeholder UI copy from homepage/footer and kept the Join Now flow as the active community/support path.
- Verification passed: `npm run typecheck`, `npm run lint`, `npm run validate:unanswered-files`, `npm run build`, old URL scans for `www.cockroachwatchindia.online`, `cwi-ten.vercel.app`, `vercel.app`, and localhost production URLs, plus local HTTP smoke checks on `/`, `/live-newsroom`, lead article, `/support`, `/submit`, `/india-unanswered-files`, Ladakh file, `/archive`, `/admin/login`, `/admin/settings/join`, `/sitemap.xml`, and `/robots.txt`.
- Browser plugin failed to initialize due a missing kernel asset path, so UI interaction was verified via local headless Chrome/CDP: desktop floating button present, modal opens, QR optimized image loads, links render, drag snaps left and saves `cwi-floating-join-side=left`, and mobile 390px viewport keeps button inside screen and opens the modal.
- Dev server is running locally at `http://localhost:3000` for review.

2026-05-31 Join Now visibility fix:
- User reported Join Now was not visible in light mode and not appearing on every page.
- Hardened `.cwi-floating-join` with dedicated CSS that forces visible navy/light-mode-safe styling, white text, stronger blue/amber glow, and z-index `2147483000` above all page layers.
- Hardened modal overlay z-index with `.cwi-join-modal-layer` so the Join panel opens above dark/light pages and page-specific sections.
- Verified in headless Chrome/CDP with light theme forced: floating Join button present, visible, in-bounds, white text, high z-index, and navy gradient on `/`, `/live-newsroom`, `/support`, `/submit`, `/india-unanswered-files`, `/india-unanswered-files/ladakh-sixth-schedule-statehood`, `/archive`, and `/about`.
- Verification passed after fix: `npm run typecheck`, `npm run lint`, and `npm run build`.

2026-05-31 localhost CSS 404 fix:
- User screenshot showed unstyled homepage with huge image/text and no visible Join Now. Root cause was not component code: localhost HTML linked `/_next/static/css/app/layout.css?...`, but the running Next server returned 404 for that CSS chunk.
- Found duplicate/stale Node/Next server state on port 3000. Stopped the stale local Next processes, removed `.next`, and restarted one clean `npm run dev` server.
- Verified CSS now returns HTTP 200 `text/css` with length ~114 KB, homepage HTML includes `cwi-floating-join`, and headless Chrome reports active stylesheets, normal nav/hero sizing, light background, and visible floating Join button.
- Dev server is running cleanly at `http://localhost:3000`; if browser still shows broken layout, hard refresh with Ctrl+F5 or clear site cache for localhost.

2026-05-31 navbar Join Now removal:
- Removed the desktop navbar Join Now trigger and mobile menu Join Now entry from `components/Navbar.tsx` per user request.
- Kept the global floating Join Now button/modal, homepage/footer Join CTAs, and config-backed support flow intact.
- Verified `npm run typecheck` and `npm run lint` pass; `components/Navbar.tsx` no longer contains `JoinNowTrigger`, `Join Now`, or `join-now`.

2026-05-31 Join modal light-mode contrast fix:
- User reported Join Now still looked wrong in light mode; screenshot showed the global Join modal washed out on `/live-newsroom`.
...
## 2026-06-08 Live Newsroom 30 May-8 June update
- User attached research-only prompt for CWI Live Newsroom updates from 30 May 2026 to 8 June 2026, with strict requirements: no UI/UX/color/font changes, no fake sources, no unsupported claims, cautious labels for reported/developing claims, and no claim that Sonam Wangchuk is officially becoming Education Minister.
- `data/live-newsroom-june-2026.ts` added six approved June 8 public records: peaceful student protest, NEET/NTA accountability, peaceful protest advisory, Sonam Wangchuk education-reform public discussion, CWI public education minister poll, and CBSE OSM/DoPT probe accountability roundup.
- `data/live-newsroom.ts` now imports the June records ahead of existing base records, extends verification/status labels for public sentiment and official-clarification language, and adds a peaceful-protest public advisory.
- `components/PollCard.tsx` now implements the requested education-leadership poll with localStorage/device vote lock, local vote timestamp, results after voting, total votes, percentage bars, privacy disclaimer, and optional admin reset/archive controls.
- `app/live-newsroom/page.tsx` now includes a peaceful-protest paragraph/badge and `/live-newsroom#education-poll` poll section; homepage `app/page.tsx` includes the compact "Live from CWI Newsroom" highlight and poll CTA.
- `scripts/generate-static-seo.mjs` now reads the June newsroom data module and uses `2026-06-08` lastmod. `docs/cwi-live-newsroom-social-captions-2026-06-08.md` stores the 10 requested social captions.
...
2026-06-16 reusable schema + robots pass:
- User requested explicit reusable SEO components for NewsArticle, Organization, BreadcrumbList, robots.txt, and sitemap confirmation.
- Added `components/seo/NewsArticleSchema.tsx`; it outputs `NewsArticle` JSON-LD and a section-aware `BreadcrumbList`. It accepts headline, dates, description, URL, image URL, author name, section name, and section URL.
- Added `components/seo/OrganizationSchema.tsx`; mounted it on `app/page.tsx` so the homepage emits the requested `NewsMediaOrganization` schema with sameAs links, founding date, masthead, publishing principles, corrections policy, and ethics policy.
- Removed the duplicate global Organization JSON-LD from `app/layout.tsx`; kept global WebSite schema there with a publisher object.
- Replaced inline article JSON-LD on Live Newsroom detail pages, Archive article pages, and India Unanswered Files detail pages with `NewsArticleSchema`, using correct section breadcrumbs: Home -> Live Newsroom/Archive/India Unanswered Files -> article title.
- Kept FAQPage JSON-LD on Unanswered Files detail pages and listing schemas on listing pages because they are separate from article schema.
- Updated `scripts/generate-static-seo.mjs` and regenerated `public/robots.txt`. Robots now allows public routes and blocks only `/admin/`, `/api/`, `/drafts/`, `/test/`, and the removed archive draft slug; sitemap points to `https://cockroachwatchindia.online/sitemap.xml`.
- Verification: `npm run lint` passed; `npm run build` passed and regenerated 125 sitemap URLs / 212 static pages. Generated HTML spot checks confirmed Organization schema on home, NewsArticle + BreadcrumbList on article pages, no `https://www.cockroachwatchindia.online`, and robots/sitemap output is present.

2026-06-16 Phase 2 SEO validation and Google News metadata:
- User requested Phase 2 SEO work without re-implementing Phase 1. Validated the live deployment first and found a production redirect loop: Vercel redirects apex `cockroachwatchindia.online` to `www`, while app middleware redirected `www` back to apex. To stop the app-side half of the loop, removed `www.cockroachwatchindia.online` from `middleware.ts` redirect hosts. Canonicals/sitemap still use non-www; Vercel domain primary must be corrected externally to apex to fully align production.
- Added article Google News metadata through `lib/seo.ts`: article pages now emit `news_keywords`, `article:published_time`, `article:modified_time`, and `article:section`; `openGraph` article metadata receives section and tags.
- Updated Live Newsroom, Archive, and India Unanswered Files article metadata calls to pass section/category and tags.
- Changed schema components from `next/script` to native `<script type="application/ld+json">` because generated HTML validation showed `next/script` did not produce directly grep-able JSON-LD scripts in static HTML. `NewsArticleSchema`, `OrganizationSchema`, and `WebPageSchema` now emit direct JSON-LD.
- Added `components/seo/WebPageSchema.tsx` and mounted page schema on `/about` (`AboutPage`), `/contact` (`ContactPage`), `/editorial-policy`, `/corrections`, `/credit-policy`, `/privacy-policy`, `/terms`, `/submit`, and `/support`.
- Updated `scripts/generate-static-seo.mjs`: filters empty slugs, explicitly includes `data/live-newsroom-june-14-2026.ts`, keeps non-www `https://cockroachwatchindia.online`, sets homepage priority `1.0`, and changes `/live-newsroom` priority to `0.9`.
