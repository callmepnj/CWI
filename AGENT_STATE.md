...
- 2026-05-25 approval UI clarity fix in progress: user saw "Publishing blocked. Human approval required." after using "Approve Social Only". Publish AI is correctly strict: only `approved`/Approve Publish can publish, and items without `article_draft_id` cannot publish. Updating approval buttons to send canonical statuses, disabling Run Publish AI until status is exactly `approved` and an article draft is attached, and adding explanatory copy on approval cards.
